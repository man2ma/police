           ...
           <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php }elseif($offence->ar_no != 'Haijawekwa'){ ?>
                        <?php echo $offence->ar_no; ?>
                        <?php }else{ ?>
                        <?php if(($controller =='cro' || $controller =='ocs') && ($this->session->userdata('station_classid')) ==1){ ?>
                        ...
                        <?php if($controller =='ocd' || $controller == 'occid'){ ?>
...
<div class='row'>
    ...
    <ul class="nav nav-tabs">
      ...
      <li><a href="#tab7" data-toggle="tab">Vyombo vilivyohusika</a></li>
      <?php if($this->session->userdata('controller') =='cro'){ ?>
...
<!--vyombo vilivyohusika tab-->
          <div class="tab-pane" id="tab7">
              <br/>
               <?php if($offence_status<12){ ?>
              <button  class="btn btn-info"  data-toggle="modal" data-target="#vyombo">Ongeza Vyombo</button>
               <?php } ?>
              ...
              <br/><?php echo $this->session->flashdata('message'); ?>
               <div class="row">
                <!-- If there is No any Complainant Display No complainant message -->
                <?php if(!$mhusikainfo){
                    $this->load->view('common/message/noChombo');
                }else {
                //If there is Complainant display them all
                    $this->load->view('common/display/view_mhusika', $data);
                }
...
<div class="modal fade" id="assignIR" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Weka AR-No</h4>
      </div>
      <div class="modal-body">
       <?php echo form_open($this->session->userdata('controller').'/assignAR/'.$offence_id); ?>
        <div class="form-group">
...
<!-- Vyombo Modal -->
<div class="modal fade" id="vyombo" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel"><i class="glyphicon glyphicon-cog"></i> Ongeza Chombo</h4>
      </div>
      <div class="modal-body">
        
   <?php
    $namba_settings = array(
        'name'        => 'namba',
        'id'          => 'gari_namba',
        'placeholder' => 'Namba ya Chombo',
        'class' => 'form-control'  
    );
    
    $chasis_settings = array(
        'name'        => 'chasis',
        'id'          => 'chasis',
        'placeholder' => 'Chasis namba',
        'class' => 'form-control'  
    );
    
    $rangi_settings = array(
        'name'        => 'rangi',
        'id'          => 'rangi',
        'placeholder' => 'Rangi ya chombo',
        'class' => 'form-control'  
    );
...
<input type="hidden" name="" value="<?php echo $this->session->userdata('controller'); ?>" id="controller"/>
          
<div class="form-group">
    <label>Aina ya Chombo</label>
       <?php 
       $js = 'id="chombo" class="form-control"';
       echo form_dropdown('chombo',$chombo_type,'large', $js); 
       ?>
</div> 
...
<div class="form-group">
    <label>Namba ya chombo</label>
    <?php 
      echo form_input($namba_settings);
    ?>
</div>
          
<div class="form-group">
    <label>Chasis Namba</label>
    <?php 
      echo form_input($chasis_settings);
    ?>
</div>
          
<div class="form-group">
    <label>Rangi ya chombo</label>
    <?php 
      echo form_input($rangi_settings);
    ?>
</div>
