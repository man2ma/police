...
class Occid extends CI_Controller{
 ...
 public function __construct(){         
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           //pagination
           $config['base_url'] = base_url().$this->uri->segment(1).'/occid/viewOffence/'.$query_id.'/'.$sort_by.'/'.$sort_order.'/';
    ...
    public function filterOffence(){
       ...
       redirect("occid/viewOffence/$query_id");  
    }
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignIR($offence_id){
        ...
        if($assign_ir){
            redirect('occid/viewOneOffence/'.$offence_id);
        }
    }
...
public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'occid')
             {
            redirect('home');
            die();
             }
