...
class Csuhqx extends CI_Controller{
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           $query_array = array(
             ...
             'offence_statusid' => $this->input->get('offence_statusid'),
              'hqinternalstatus_no' => 'x'
             );
           $result = $this->csuhqx_model->getOffence($query_array,$limit,$offset,$sort_by,$sort_order);
           ...
           //pagination
           $config['base_url'] = base_url().$this->uri->segment(1).'/csuhqx/viewOffence/'.$query_id.'/'.$sort_by.'/'.$sort_order.'/';
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignDossierno($offence_id){
        ...
        if($assign_dossier_no){
            redirect('csuhqx/viewOneOffence/'.$offence_id);
        }
    }
...
public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'csuhqx')
             {
            redirect('home');
            die();
             }
