...
class Hq extends CI_Controller{
 ...
 public function __construct(){         
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           $result = $this->hq_model->getOffence($query_array,$limit,$offset,$sort_by,$sort_order);
           ...
           //pagination
           $config['base_url'] = base_url().$this->uri->segment(1).'/hq/viewOffence/'.$query_id.'/'.$sort_by.'/'.$sort_order.'/';
    ...
    public function filterOffence(){
       ...
       redirect("hq/viewOffence/$query_id");  
    }
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignDCI($offence_id){
        ...
        if($assign_dci){
            redirect('hq/viewOneOffence/'.$offence_id);
        }
    }
...
public function report_Majorcrime(){
     ...
     public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'hq')
             {
            redirect('home');
            die();
             }
