...
class Ocs extends CI_Controller{
 ...
 public function __construct(){         
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           $result = $this->ocs_model->getOffence($query_array,$limit,$offset,$sort_by,$sort_order);
   
           //Special for Search
           $search=  $this->input->post('search');
           $result_search = $this->ocs_model->getOffenceSearch($search);
            json_encode ($result_search);
           //end of Search
           ...
           //pagination
           $config['base_url'] = base_url().$this->uri->segment(1).'/ocs/viewOffence/'.$query_id.'/'.$sort_by.'/'.$sort_order.'/';
    ...
    public function filterOffence(){
       ...
       redirect("ocs/viewOffence/$query_id");  
    }
       ...
       public function viewOneOffence($offence_no){
           ...
           $data['oneoffence']  = $this->ocs_model->getOneOffence($offence_no);
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignIR($offence_id){
        ...
        if($assign_ir){
            redirect('ocs/viewOneOffence/'.$offence_id);
        }
    }
  public function transfer_to_District($offence_id){
     ...
     public function LoadCourt(){
      ...
      $this->load->view('common/form/wardCourt',$data);
...
public function report_Majorcrime(){
   ...
   public function viewFullOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
    ...
    public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'ocs')
             {
            redirect('home');
            die();
             }
