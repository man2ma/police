...
class Igp extends CI_Controller{
 ...
 public function __construct(){         
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           $result = $this->igp_model->getOffence($query_array,$limit,$offset,$sort_by,$sort_order);
           ...
           //pagination
           $config['base_url'] = base_url().$this->uri->segment(1).'/igp/viewOffence/'.$query_id.'/'.$sort_by.'/'.$sort_order.'/';
    ...
    public function filterOffence(){
       ...
       redirect("igp/viewOffence/$query_id");  
    }
...
//                 );
            $data['dci_settings'] = array(
                    ...
                    'data-mask'=>"DCI/SCR/999999/".date('Y'),
                    'class' => 'form-control'  
                 );
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignDCI($offence_id){
        ...
        if($crimetypeid==1)
        {
          $data['igpinternalstatus_no'] ='x';   
        }
        if($crimetypeid==2)
        {
          $data['igpinternalstatus_no'] ='y';   
        }
        if($crimetypeid==3)
        {
          $data['igpinternalstatus_no'] ='z';   
        }
        
        $assign_dci= $this->common_model->update($data,'offence','offence_no',$offence_id);
        if($assign_dci){
            redirect('igp/viewOneOffence/'.$offence_id);
        }
    }
    public function transfer_to_Dpp($offence_id){
        $data['offence_statusid'] = 7;
...
public function report_Majorcrime(){
         ...
         $data['todate']=$todate;
         $data['tukiostatus']=2;
         $data['region_info'] =$this->common_model->getPoliceRegion();  
     ...
     public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'igp')
             {
            redirect('home');
            die();
             }
