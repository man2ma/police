...
class Rpc extends CI_Controller{
 ...
 public function __construct(){         
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           $result = $this->rco_model->getOffence($query_array,$limit,$offset,$sort_by,$sort_order);
           ...
           //pagination
           $config['base_url'] = base_url().$this->uri->segment(1).'/rpc/viewOffence/'.$query_id.'/'.$sort_by.'/'.$sort_order.'/';
    ...
    public function filterOffence(){
       ...
       redirect("rpc/viewOffence/$query_id");  
    }
       ...
       public function viewOneOffence($offence_no){
           ...
           //IR text input is loaded here
            ...
            $data['rco_settings'] = array(
                    'name'        => 'rco',
                    'id'          => 'rco',
                    'type'        => 'rco',
                    'placeholder' => 'RCO Namba',
                    'data-mask'=>"aaa/RCO/999999/".date('Y'),
                    'class' => 'form-control'  
                 );
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignRCO($offence_id){
        $data['rco_no'] = $this->input->post('rco');
        $data['offence_statusid'] =4;
        $assign_rco = $this->common_model->update($data,'offence','offence_no',$offence_id);
        if($assign_rco){
            redirect('rpc/viewOneOffence/'.$offence_id);
        }
    }

   public function transfer_to_Hq($offence_id){
        $data['offence_statusid'] = 5;
   ...
   public function LoadCourt(){
      ...
      $this->load->view('common/form/regionCourt',$data);
 ...
 public function view_kesiImefungwa(){
   ...
   public function report_Majorcrime(){
         ...
         $data['todate']=$todate;
         $data['tukiostatus']=2;
         $data['district_info'] =$this->common_model->getReportpolicedistrict();  
...
public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'rpc')
             {
            redirect('home');
            die();
             }
