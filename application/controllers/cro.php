...
public function viewTaarifa($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
    ...
    public function newOffence(){
       ...
       public function getCrimeCategory(){
        ...
        public function getVehicleSubCat() {
         $data['vehicle_category']=$this->common_model->getVehiclesubcategory($val=1);
         $this->load->view('common/form/vehicle_subcat',$data);
      ...
      public function getCrimeSubCategory(){
                ...
                $data['ar_settings'] = array(
                    'name'        => 'ar',
                    'id'          => 'ar',
                    'type'        => 'text',
                    'value'=>$this->getARSeries(),
                    //'readonly'=>'true',
                    'class' => 'form-control'  
                 );
                ...
                $sababu_ajali = array(17,18,19,20,21,22,23,24,25);
                $data['sababu_ajali'] = $this->common_model->getCrimesubCategoryType($sababu_ajali);
          //---Start Makosa dhidi ya Binadamu
           ...
           //---Start of Makosa Barabarani
            elseif($crime_cat ==57){
               
                $this->load->view('common/form/barabarani/ajali',$data);
           }
        }
        ...
        public function getARSeries() {
            
            $station_initial = $this->session->userdata('station_initial'); 
            
            //select to get the last rb in the table Taarifa
            $this->db->select('ar_no');
            $this->db->from('offence');
            $this->db->where('date_format(date_reported,"%Y")="'.date('Y').'"');
            $this->db->not_like('ar_no','Haijawekwa');
            $this->db->where('station_id',$this->session->userdata('station_id'));
            ...
            $totalseries1 = $max_ar + 1;
            $totalseries = "0000".$totalseries1;

            $realseries = substr($totalseries, -6);
            $ar = $station_initial."/AR/".$realseries."/".date('Y');
                    return $ar;
            }        
...
//           $data['mauaji_vikongwe']  = $this->input->post('mauajivikongwe');
    ...
    public function filterOffence(){
     ...
     public function viewOneOffence($offence_no){
           ...
           $data['chombo_type'] = $this->common_model->getVehicle();
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignIR($offence_id){
     ...
     public function assignAR($offence_id){
        $data['offence_statusid'] = 2;
        $data['ar_no'] = $this->input->post('ar');
        $assign_ar = $this->common_model->update($data,'offence','offence_no',$offence_id);
        if($assign_ar){
            redirect('cro/viewOneOffence/'.$offence_id);
        }
    }
