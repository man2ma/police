...
class Csuhqy extends CI_Controller{
       ...
       public function viewOffence($query_id = 0, $sort_by = 'request_id', $sort_order = 'desc', $offset = 0){
           ...
           $query_array = array(
             ...
             'offence_statusid' => $this->input->get('offence_statusid'),
              'hqinternalstatus_no' => 'y'
             );
           $result = $this->csuhqy_model->getOffence($query_array,$limit,$offset,$sort_by,$sort_order);
...
elseif(isset ($_POST['crop'])){
    ...
    public function assignDCI($offence_id){
        ...
        if($assign_dci){
            redirect('csuhqy/viewOneOffence/'.$offence_id);
        }
    }
...
public function isLoggedIn() {
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != true || $isPosition != 'csuhqy')
             {
            redirect('home');
            die();
             }
