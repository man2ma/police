...
$(function(){
    $('#chombo').live('change',function(){
         var controller_name = $('#controller').val();
         var namba=$('#chombo').val();
        if(namba=== ''){
        $('#space').load(CI.base_url_ajax+"/getSpaceHolder");  
          $('#space').hide();
        }
        if(namba==1){
         var dataString = 'chombo='+ this.value; 
         //alert(dataString); die;
          $.ajax({  
                 type: "POST",  
                 url:  CI.base_url_ajax+controller_name+"/getVehicleSubCat",  
                 data: dataString,  
                 success: function(data) { 
                    $('#space').html(data);
                    $('#space').show();
